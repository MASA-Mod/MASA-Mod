...
public class MarsChunkGenerator extends ChunkGenerator {
    ...
    public static final Codec<MarsChunkGenerator> CODEC = RecordCodecBuilder.create(instance ->
            instance.group(
                    RegistryLookupCodec.create(Registry.BIOME_REGISTRY).forGetter(MarsChunkGenerator::getBiomeRegistry),
                    SETTINGS_CODEC.fieldOf("settings").forGetter(MarsChunkGenerator::getTutorialSettings)
            ).apply(instance, MarsChunkGenerator::new));

    private final Settings settings;

    public MarsChunkGenerator(Registry<Biome> registry, Settings settings) {
        super(new MarsBiomeProvider(registry), new StructureSettings(false));
        this.settings = settings;
        masa.LOGGER.info("Chunk generator settings: " + settings.baseHeight() + ", " + settings.horizontalVariance() + ", " + settings.verticalVariance());
    }
    ...
    public Registry<Biome> getBiomeRegistry() {
        return ((MarsBiomeProvider)biomeSource).getBiomeRegistry();
    }
    ...
    public ChunkGenerator withSeed(long seed) {
        return new MarsChunkGenerator(getBiomeRegistry(), settings);
    }
